# Chunking - After splitting and assigning pos, we need to know what the text is
# talking about (subject) and finding words that modify/effect it (predicate) 
# Chunking helps to group strings of words (based on type)
import nltk
from nltk.corpus import state_union
from nltk.tokenize import PunktSentenceTokenizer # PunktSentenceTokenizer is a ML tokenizer!

def process_content(): 
	try:
		for i in tokenized: 
			words = nltk.word_tokenize(i)
			tagged = nltk.pos_tag(words)

			'''Here begins the Chunking'''
			chuckGram = r"""Chuck: {<RB.?>*<VB.?>*<NNP>+<NN>?}"""

			chuckParser = nltk.RegexpParser(chuckGram)
			chunked = chuckParser.parse(tagged)

			chunked.draw()
			# print(chunked)


	except Exception as e:
		print(str(e))


# get text
train_text = state_union.raw("2005-GWBush.txt")
sample_text = state_union.raw("2006-GWBush.txt")

# apply train text
custom_sent_tokenizer = PunktSentenceTokenizer(train_text)

# apply tokenizer on text
tokenized = custom_sent_tokenizer.tokenize(sample_text)

# call function to tokenize and chunk
process_content()

